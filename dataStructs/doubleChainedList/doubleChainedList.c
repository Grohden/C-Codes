...
DoublyChainedListHead *initDoublyChain() {
  DoublyChainedListHead *head =
      (DoublyChainedListHead *)calloc(1, sizeof(DoublyChainedListHead));
...
int *getDoublyChainDataAt(DoublyChainedListHead *head, int elementAt) {
  if (isDoublyChainEmpty(head)) {
    ...
    println("Nao e possivel pegar o item, index %d fora dos limites",
            elementAt);
    return NULL;
  } else {
...
bool removeFromDoublyChainStart(DoublyChainedListHead *head) {
  ...
  if (getDoublyChainLength(head) == 1) {
    ...
    DoublyChain *removedNode = head->start;

    DoublyChain *tailNode = removedNode->previous;
    DoublyChain *headNode = removedNode->next;
...
int getDoublyChainLength(DoublyChainedListHead *head) { return head->length; }
